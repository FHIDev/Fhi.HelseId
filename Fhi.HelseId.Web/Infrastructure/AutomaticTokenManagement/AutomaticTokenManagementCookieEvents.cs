...
public class AutomaticTokenManagementCookieEvents : CookieAuthenticationEvents
{
    ...
    public override async Task ValidatePrincipal(CookieValidatePrincipalContext context)
    {
        ...
        _logger.LogTrace("Starting checking token and possible refresh.");
        await _tokenConfig.CookieEvents.ValidatePrincipal(context);

        var tokens = context.Properties.GetTokens()?.ToList();
        if (tokens == null || !tokens.Any())
        {
            _logger.LogError("No tokens found in cookie properties. SaveTokens must be enabled for automatic token refresh.");
            return;
        }
        ...
        {
            _logger.LogError("No refresh token found in cookie properties. A refresh token must be requested and SaveTokens must be enabled.");
            return;
        }
        ...
        if (expiresAt == null)
        {
            _logger.LogError("No expires_at value found in cookie properties.");
            return;
        }
        ...
        _logger.LogTrace("Using current token {token} expires at {expires}.", rfValue, dtExpires);
        ...
        _logger.LogTrace("ValidatePrincipal: expires_at: {dtExpires}, refresh_before: {refreshBeforeExpiration}, refresh_at: {dtRefresh}, now: {utcNow}, refresh_token: {refreshToken}.",
            dtExpires,
            _tokenConfig.RefreshBeforeExpiration,
            dtRefresh,
            utcNow,
            refreshToken.Value);
        ...
        if (_helseIdConfig.UseApis)
        {
            if (dtRefresh < utcNow)
            {
                ...
                _logger.LogTrace("Should refresh: {shouldRefresh}, No Of tokens in PendingRefreshTokenRequests {count} ",
                    shouldRefresh,
                    PendingRefreshTokenRequests.Count);
                ...
                if (shouldRefresh)
                {
                    try
                    {
                        ...
                        _logger.LogTrace("SignInAsync now as it expires at: {newExpiresAt}.", newExpiresAt);
                        _logger.LogTrace("Refresh tokens: Current {current}, New {new}.", rfValue, response.RefreshToken);
                ...
                {
                    _logger.LogTrace("Refresh token already requested, value {refreshtoken}.", refreshToken.Value);
                }
            }
            else
            {
                _logger.LogTrace("No need to refresh token yet.");
            }
        }
        else
        {
            _logger.LogTrace("No Apis in configuration.");
        }
